          ...
          {ROADMAP.map((week, index) => (
            <div
              ...
              className={`bg-white rounded-2xl shadow-md p-6 cursor-pointer transition-all duration-300 hover:shadow-lg ${
                expandedWeek === index ? 'ring-2 ring-teal-500' : ''
              }`}
            ...
            >
              <div className="flex items-center mb-4">
                ...
                <h3 className="ml-3 text-lg font-semibold text-gray-900">{week.week}</h3>
              </div>

              <div className="space-y-3">
                {week.sessions.map((session) => (
                  <div key={session.id} className="border-l-2 border-teal-200 pl-3">
                    <h4 className="font-medium text-gray-900 text-sm">{session.title}</h4>
                    <p className="text-xs text-gray-600 mt-1">{session.outcome}</p>
                  </div>
                ))}
              </div>
              ...
              {expandedWeek === index && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="bg-teal-50 rounded-lg p-3">
                    <h5 className="font-medium text-teal-900 text-sm mb-1">Assignment:</h5>
                    <p className="text-sm text-teal-700">{week.assignment}</p>
                  </div>
                  {index === 3 && (
                    <div className="mt-3 bg-yellow-50 rounded-lg p-3">
                      <h5 className="font-medium text-yellow-900 text-sm mb-1">🎯 Capstone Week</h5>
                      <p className="text-sm text-yellow-700">Present your final project and get feedback</p>
                    </div>
                  )}
                </div>
              )}
